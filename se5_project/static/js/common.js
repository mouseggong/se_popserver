...
function get_news_list(keyword,callback){ //콜백함수 들고간다.
	$.get(url+"/search/news/"+keyword, function(data) { 
...
function get_news_list_byreporter(keyword,parentname,nodename,callback){ //콜백함수 들고간다.
	$.get(url+"/search/tree/"+keyword+"/"+parentname+"/"+nodename, function(data) { 
	//get 방식 ajax요청(파일의 위치, 함수)
   //첫번째 매개변수는 요청 주소이고
   //두번째 매개변수는 성공했을 때 호출되는 메서드이며
   //이 메서드의 매개변수가 가져온 데이터입니다.
